...
async def chat_completion_stream_generator(
    ...
    for i in range(n):
        ...
        async for content in generate_completion_stream(gen_params, worker_addr):
            total_tokens=content['usage']['completion_tokens']
            if content["error_code"] != 0:
    ...
    for finish_chunk in finish_stream_events:
        model_dump_data = chunk.model_dump(exclude_unset=True)
        end_time=time.time()
        total_time = round(end_time - start_time,2)
        tokens_per_second = round(total_tokens / total_time,2)
        model_dump_data['total_tokens'] = total_tokens  
        model_dump_data['total_time'] = total_time
        model_dump_data['tokens_per_second'] = tokens_per_second
        # 使用 json.dumps 将字典转换为JSON字符串  
        json_string = json.dumps(model_dump_data, ensure_ascii=False)

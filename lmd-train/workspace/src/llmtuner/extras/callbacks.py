...
from .java_rsa_authorization import add_auth_headers
...
class LogCallback(TrainerCallback):
    def __init__(self, runner=None, task_dict=None):
        self.runner = runner
    ...
    @add_auth_headers
    def business_file_address_query(self, logs, **kwargs) -> dict:
        url = self.task_dict["api_url"]

        payload = {
            "trainLogList": [{
                "ts": datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"),
                "currentSteps": int(logs["current_steps"]),
                "elapsedTime": logs["elapsed_time"],
                "epoch": logs["epoch"],
                "epochEnd": 0,
                "epochStart": 0,
                "evalLoss": logs["eval_loss"],
                "learningRate": logs["learning_rate"],
                "loss": logs["loss"],
                "lossEnd": 0,
                "lossStart": 0,
                "percentage": logs["percentage"],
                "percentageEnd": 0,
                "percentageStart": 0,
                "predictLoss": logs["predict_loss"],
                "remainingTime": logs["remaining_time"],
                "remainingTimeEnd": "",
                "remainingTimeStart": "",
                "accuracy": logs["accuracy"],
                "reward": logs["reward"],
                "taskId": int(self.task_dict["task_id"]),
                "totalSteps": int(logs["total_steps"])
            }]
        }
        ...
        headers.update(custom_headers)  # 合并定制headers到默认headers中
        try:
    ...
    def on_log(self, args: "TrainingArguments", state: "TrainerState", control: "TrainerControl", **kwargs) -> None:
        ...
        logs = dict(
            ...
            api_url=self.task_dict["api_url"]
        )
        ...
        with open(os.path.join(args.output_dir, "trainer_log.jsonl"), "a", encoding="utf-8") as f:
            ...
            # logger.info("开始输出啦")
            self.business_file_address_query(logs)
